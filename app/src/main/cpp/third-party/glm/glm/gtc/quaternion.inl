...
GLM_FUNC_QUALIFIER tquat<T, P>::tquat(tvec3<T, P> const& eulerAngle) {
    double half_x = eulerAngle.x * 0.5;
    double half_y = eulerAngle.y * 0.5;
    double half_z = eulerAngle.z * 0.5;
    tvec3<T, P> c{(T)cos(half_x), (T)cos(half_y), (T)cos(half_z)};
    tvec3<T, P> s{(T)sin(half_x), (T)sin(half_y), (T)sin(half_z)};
...
template <typename U>
GLM_FUNC_QUALIFIER tquat<T, P>& tquat<T, P>::operator+=(tquat<U, P> const& q) {
    return (*this = detail::compute_quat_add<T, P, detail::is_aligned<P>::value>::call(
                *this, tquat<T, P>(q)));
}
...
template <typename U>
GLM_FUNC_QUALIFIER tquat<T, P>& tquat<T, P>::operator-=(tquat<U, P> const& q) {
    return (*this = detail::compute_quat_sub<T, P, detail::is_aligned<P>::value>::call(
                *this, tquat<T, P>(q)));
}
...
GLM_FUNC_QUALIFIER tquat<T, P>& tquat<T, P>::operator*=(U s) {
    return (*this = detail::compute_quat_mul_scalar<T, P, detail::is_aligned<P>::value>::call(
...
template <typename U>
GLM_FUNC_QUALIFIER tquat<T, P>& tquat<T, P>::operator/=(U s) {
    return (*this = detail::compute_quat_div_scalar<T, P, detail::is_aligned<P>::value>::call(
                *this, static_cast<U>(s)));
}
...
GLM_FUNC_QUALIFIER tquat<T, P> cross(tquat<T, P> const& q1, tquat<T, P> const& q2) {
    return tquat<T, P>(q1.w * q2.w - q1.x * q2.x - q1.y * q2.y - q1.z * q2.z,
...
/*
        ...
        template <typename T, precision P>
        GLM_FUNC_QUALIFIER tquat<T, P> mix(tquat<T, P> const & x, tquat<T, P> const & y, T const &
   a)
        {
                if(a <= T(0)) return x;
                if(a >= T(1)) return y;
...
GLM_FUNC_QUALIFIER tquat<T, P> rotate(tquat<T, P> const& q, T const& angle, tvec3<T, P> const& v) {
    ...
    T const Sin = (T)sin(AngleRad * 0.5);

    return q * tquat<T, P>((T)cos(AngleRad * 0.5), Tmp.x * Sin, Tmp.y * Sin, Tmp.z * Sin);
    // return gtc::quaternion::cross(q, tquat<T, P>(cos(AngleRad * T(0.5)), Tmp.x * fSin, Tmp.y *
    // fSin, Tmp.z * fSin));
}
...
GLM_FUNC_QUALIFIER tvec3<T, P> axis(tquat<T, P> const& x) {
    ...
    if (tmp1 <= static_cast<T>(0)) return tvec3<T, P>(0, 0, 1);
...
GLM_FUNC_QUALIFIER tquat<T, P> angleAxis(T const& angle, tvec3<T, P> const& v) {
    ...
    T const s = (T)glm::sin(a * 0.5);

    Result.w = (T)glm::cos(a * 0.5);
    Result.x = v.x * s;
    Result.y = v.y * s;
    Result.z = v.z * s;
    return Result;
